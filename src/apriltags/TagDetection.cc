...
TagDetection::TagDetection()
  : good(false), obsCode(), code(), id(), hammingDistance(), rotation(), p(),
    cxy(), observedPerimeter(), homography(), hxy(), outlier(false), residuals() {
  homography.setZero();
}
...
float TagDetection::getXYOrientation() const {
  ...
  return ! std::isnan(float(orient)) ? orient : 0.0f;
}

std::pair<float,float> TagDetection::interpolate(float x, float y) const {
  float z = (float)homography(2,0)*x + (float)homography(2,1)*y + (float)homography(2,2);
  if ( z == 0 )
...
float TagDetection::pixelArea() const
{
    float A = 0.0f;
    const int index[4][2] = {{0, 3}, {3, 2}, {2, 1}, {1, 0}};
    int i;

    // Shoelace formula with precalculated indices in index table
    for (i = 0; i < 4; i++)
    {
        const float Re_0 =  p[index[i][0]].first;
        const float Re_1 =  p[index[i][1]].first;
        const float Ho_0 = -p[index[i][0]].second;
        const float Ho_1 = -p[index[i][1]].second;
        A += (Re_0 + Re_1)*(Ho_0 - Ho_1);
    }
    A *= 0.5f;
    return A;
}
...
void TagDetection::draw(cv::Mat& image, int color) const {
  ...
  // put this stuff into a loop, boy

  cv::Mat img2 = image.clone();
  const cv::Scalar overlay_color[] = { cv::Scalar(0,255,0),
                                       cv::Scalar(0,255,255),
                                       cv::Scalar(0,0,255),
                                       cv::Scalar(255,0,255) };
  cv::Point pts[4] = { cv::Point(p1.first+1, p1.second-1),
                       cv::Point(p2.first-1, p2.second-1),
                       cv::Point(p3.first-1, p3.second+1),
                       cv::Point(p4.first+1, p4.second+1) };
  cv::fillConvexPoly(img2,
                     pts, sizeof(pts)/sizeof(pts[0]),
                     overlay_color[color], cv::LINE_AA);
  const double alpha = 0.5;
  cv::addWeighted(image, 1 - alpha, img2, alpha, 0, image);

  /* draw bounding lines */
  cv::line(image, cv::Point2f(p1.first, p1.second), cv::Point2f(p2.first, p2.second), cv::Scalar(70,70,70,0), 1, cv::LINE_AA );
  cv::line(image, cv::Point2f(p2.first, p2.second), cv::Point2f(p3.first, p3.second), cv::Scalar(70,70,70,0), 1, cv::LINE_AA );
  cv::line(image, cv::Point2f(p3.first, p3.second), cv::Point2f(p4.first, p4.second), cv::Scalar(213,118,40,0), 1, cv::LINE_AA );
  cv::line(image, cv::Point2f(p4.first, p4.second), cv::Point2f(p1.first, p1.second), cv::Scalar(70,70,70,0), 1, cv::LINE_AA );

  // mark center
  // cv::circle(image, cv::Point2f(cxy.first, cxy.second), 3, cv::Scalar(255,255,255,0), 1, cv::LINE_AA);
  ...
  const double shadowOffset = 1;
  cv::putText(image, strSt.str(),
              cv::Point2f(cxy.first + textOffset + shadowOffset, cxy.second + textOffset + shadowOffset),
              cv::FONT_HERSHEY_PLAIN, fontScale, cv::Scalar(0,0,0));
  // text
  cv::putText(image, strSt.str(),
              cv::Point2f(cxy.first + textOffset, cxy.second + textOffset),
              cv::FONT_HERSHEY_PLAIN, fontScale, cv::Scalar(255,255,255));
  // area
  std::ostringstream strStArea;
  float area = pixelArea();
  int area_int = (int)area;
  strStArea << "A " << area_int;
  cv::putText(image, strStArea.str(),
